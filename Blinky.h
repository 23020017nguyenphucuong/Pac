...
class Blinky : public BaseObject
{
private:
	int frame_;
	int width_frame_;
...
public:
	...
	void BlinkyMove1(Map& map_data, std::pair<int,int> blinky_coor,std::pair<int,int> pac_coor);
	...
	bool isValid(int x, int y, Map& map_data);
	Node* findShortestPath(Map& map_data, std::pair<int, int> start, std::pair<int, int> target);
